...
transit_df =
  ...
  janitor::clean_names() %>%
  select(line:entry, vending, ada, exit_only) %>%
...
* Proportion of station entrances/exits without vending allow entrance is `r filter(transit_df, entry == "TRUE" | exit_only == "Yes") %>% filter(vending == "NO") %>% select(line, station_name, vending) %>% distinct(line, station_name, vending) %>% nrow() / nrow(distinct(transit_df, line, station_name))`. 
